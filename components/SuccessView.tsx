...
import { buildTwitterText } from "@/lib/utils";
...
export default function SuccessView({ message, count, onBack }: SuccessViewProps) {
  ...
  const whatsappRedirectTimer = useRef<number | null>(null);
  ...
  useEffect(() => {
    if (sessionStorage.getItem("uc_whatsapp_redirected") === "true") {
      return;
    }

    whatsappRedirectTimer.current = window.setTimeout(() => {
      sessionStorage.setItem("uc_whatsapp_redirected", "true");
      window.location.href = WHATSAPP_GROUP_URL;
    }, 1200);

    return () => {
      if (whatsappRedirectTimer.current !== null) {
        window.clearTimeout(whatsappRedirectTimer.current);
      }
    };
  }, []);
  ...
  function handleBackToWaitlist() {
    sessionStorage.removeItem("uc_whatsapp_redirected");
    onBack();
  }
  ...
  return (
    <div className={styles.successView} role="alert">
      ...
      <div className={styles.shareRow}>
        <button className={styles.shareBtn} type="button" onClick={shareWhatsApp} aria-label="Join WhatsApp group">
      ...
      <button className={styles.backLink} type="button" onClick={handleBackToWaitlist}>
        ← Back to waitlist
      </button>
